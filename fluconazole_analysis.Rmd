...
day_breaks <- c(-4, 0, 3, 6, 8, 10, 13, 15, 17, 20, 22, 25)
...
# Day relative to fluconazole administration for plotting
yii <- yii %>%
  mutate(day = as.integer(date - as_date("2024-01-03")))
```
...
bryfig1 <- ggplot(bry, aes(x = day, y = value, group = area, color = area)) +
  geom_point() +
  geom_line() +
...
(fig1a <- res %>%
  mutate(day = as.integer(date - as_date("2024-01-03"))) %>%
  filter(day < 25) %>%
  ggplot(aes(x = day, y = emmean, group = treatment, color = treatment)) +
  geom_line(alpha = 0.5) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.3, alpha = 0.5) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Average Fv/Fm of Bryopsis (all areas) in each treatment over time",
    x = "Days"
  ))
...
as_tibble(alledges) %>%
  mutate(day = as.integer(date - as_date("2024-01-03"))) %>%
  ggplot(aes(x = day, y = emmean, color = treatment, value = treatment)) +
  geom_point() + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Average Fv/Fm of Bryopsis edges in each treatment over time",
    x = "Days"
  )
...
The 1 mg/L and 5.3 mg/L treatments also don't look that different from one another, so we could combine these into "treated" vs. "untreated" (controls) to increase statistical power...
...
bry2 <- bry %>%
  mutate(trt2 = case_when(as.numeric(as.character(treatment)) == 0 ~ "control",
         ...
         trt2 = factor(trt2, levels = c("control", "treated")))
# Fit linear mixed model with treatment and date as fixed predictors, and tank as random
...
(fig1b <- as_tibble(alltrt2) %>%
  mutate(day = as.integer(date - as_date("2024-01-03"))) %>%
  filter(day < 25) %>%
  ggplot(aes(x = day, y = emmean, group = trt2, color = trt2)) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.3, alpha = 0.5) +
  geom_line(alpha = 0.5) +
  geom_point(alpha = 0.5, size = 0.75) + 
  geom_vline(xintercept = 0, lty = 2) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Average Fv/Fm of all treated vs. control Bryopsis edges over time",
    x = "Days"
  ))
...
# Figure 1 - final version with two panels
...
fig1a <- fig1a +
  labs(
    title = expression(
      paste("A. ", italic("Fv/Fm"), " of ", italic("Bryopsis"),
            " (all areas) over time")
    ),
    x = "Days",
    y = expression(italic("Fv/Fm")),
    color = "Fluconazole treatment\n(mg/L)"
  ) +
  figure1_theme
...
fig1b <- fig1b +
  labs(
    title = expression(
      paste("B. ", italic("Fv/Fm"), " of ", italic("Bryopsis"),
            " (edges) over time")
    ),
    x = "Days",
    y = expression(italic("Fv/Fm")),
...
fig1a_grob <- ggplotGrob(fig1a)
fig1b_grob <- ggplotGrob(fig1b)

# Use identical layout widths so the two panel outlines align despite
# differences in legend width.
shared_widths <- grid::unit.pmax(fig1a_grob$widths, fig1b_grob$widths)
fig1a_grob$widths <- shared_widths
fig1b_grob$widths <- shared_widths

figure1 <- cowplot::plot_grid(
  fig1a_grob, fig1b_grob,
  ncol = 1,
  align = "v"
)

figure1
ggsave(filename = "Figures/fig1.png", width = 90, height = 120, units = "mm", dpi = 300)
```
...
coralplot <- ggplot(coral, aes(x = day, y = value)) +
  geom_point() +
...
ggplot(coral, aes(x = day, y = value)) +
...
(fig2 <- as_tibble(emm) %>%
  mutate(day = as.integer(date - as_date("2024-01-03"))) %>%
  ggplot(aes(x = day, y = emmean, color = treatment, group = treatment)) +
...
Seriatopora showed an initial decline in Fv/Fm that was larger in treated corals, such that treated corals. Treatment 1 mg/L was significantly lower than control on 1/6, and Treatment 5.3 mg/L was significantly lower than control on 1/13, 1/16, and 1/25 (I think the 1/25 readings might be erroneous). Fv/Fm recovers in the treated corals by the end of the experiment and is not different than the controls.
...
fig2
ggsave(filename = "Figures/fig2.png", width = 180, height = 90, units = "mm", dpi = 300)
```
...
as_tibble(alltrt2) %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Average Fv/Fm of all treated vs. control corals over time",
    x = "Days"
  )
